...
void Frame::_CreateMenus() {
    ...
    m_menu5->Append(new wxMenuItem(m_menu5, MENU_DATA_SAVE_SELECTED, _("Save selected layer..."), wxEmptyString,
                                   wxITEM_NORMAL));
    m_menu5->AppendSeparator();
    m_menu5->Append(
...
#endif
    m_menu52->Append(
            new wxMenuItem(m_menu52, MENU_FEATURE_DELETE, _("Delete selected features") + myDelShortcut, wxEmptyString,
                           wxITEM_NORMAL));
...
void Frame::OnSaveSelectedLayer(wxCommandEvent &event) {
    vrLayerVectorOGR *myMemoryLayer = static_cast<vrLayerVectorOGR *>(m_vrViewerLayerManager->GetRenderer(
            m_vrTOC->GetSelection())->GetLayer());
...
void Frame::OnUpdateUISaveSelectedLayer(wxUpdateUIEvent &event) {
    if (m_vrTOC && m_vrTOC->GetSelection() != wxNOT_FOUND) {
        vrLayerVector *myLayer = static_cast<vrLayerVector *>(m_vrViewerLayerManager->GetRenderer(
                m_vrTOC->GetSelection())->GetLayer());
